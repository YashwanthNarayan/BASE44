...
## agent_communication:
    ...
    - agent: "testing"
      message: "🚨 UPDATED CRITICAL FINDING: NCERT Question Variety Issue ROOT CAUSE IDENTIFIED! After comprehensive testing of the improved NCERT system with enhanced error handling, discovered the real issue is AGGRESSIVE CACHING, not AI failure. The CacheUtils system caches responses for 24 hours using identical parameters (subject+topics+difficulty+count), causing the exact same 5 questions to be returned every time. Investigation showed 100% exact duplicates across all generation attempts. AI is actually working (logs show 'Generated 3 AI questions for Subject.MATH'), but cache overrides with stored responses. This completely defeats the 8+ different questions requirement. The enhanced error handling and AI model fallback are working correctly, but caching prevents question variety. IMMEDIATE FIX NEEDED: Reduce cache duration or add randomization to cache keys to allow variety while maintaining performance."
...
backend:
  - task: "NCERT Units Relevancy Issue - AI Prompt Improvements and Fallback System"
    ...
    status_history:
        ...
        - working: false
          agent: "testing"
          comment: "🚨 CRITICAL NCERT QUESTION VARIETY ISSUE CONFIRMED - AGGRESSIVE CACHING PROBLEM! Conducted comprehensive testing of the improved NCERT Units system with enhanced error handling and expanded question banks as requested in the review. DETAILED FINDINGS: ❌ QUESTION VARIETY FAILURE: All 4 test scenarios (Math Real Numbers, Math Quadratic Equations, Biology Nutrition in Plants, Chemistry Acids/Bases/Salts) FAILED the 8+ different questions requirement. Only 5 unique questions generated out of 15 attempts (33.3% variety). ❌ ROOT CAUSE IDENTIFIED: Aggressive caching in CacheUtils (backend/utils/helpers.py) with 24-hour cache duration. The cache key is generated from identical parameters (subject + topics + difficulty + question_count), causing the system to return the exact same 5 questions every time. Investigation showed 100% exact duplicates - same question text, same IDs, same hashes across all generation attempts. ✅ CONSISTENCY CHECK PARTIALLY WORKING: 2/3 units passed with 100% consistency (Math Real Numbers, Biology Nutrition in Plants), but Chemistry Acids/Bases/Salts failed with 60% consistency due to generic terms. ✅ ERROR HANDLING ROBUST: Normal operation works, rapid requests succeed (100% success rate), graceful handling of invalid subjects. ✅ AI vs FALLBACK DETECTION: System shows 88.9% AI generation, 11.1% fallback usage, indicating AI is working but cache prevents variety. ✅ LOGGING VERIFICATION: Clear evidence of AI generation in logs ('Generated 3 AI questions for Subject.MATH - Real Numbers'), but cache overrides this with stored responses. 🎯 CRITICAL ISSUE: The enhanced error handling and AI model fallback are working correctly, but the aggressive caching completely defeats the purpose of having 8+ different questions per unit. Users experience the same 5 questions repeatedly, making the system appear broken despite AI functioning properly. IMMEDIATE FIX NEEDED: Reduce cache duration or add randomization to cache keys to allow question variety while maintaining performance benefits."
